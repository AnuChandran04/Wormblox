...
BLACK = (0, 0, 0)
RED = (235, 1, 1)
RANDCOLOUR = (randint(0, 255), randint(0, 255), randint(0, 255)) #Called randint and gave it the range 0,255 so that the random number within the range of HTML colour codes. - Sanika R.
GREEN = (0, 255, 0)
DARKGREEN = (0, 155, 0)
DARKGRAY = (40, 40, 40)
BGCOLOR = (WHITE)
...
def runGame():
    ...
    apple2 = getRandomLocation() # this finds new coordinates for the apple - Sanika R.
    apple3 = getRandomLocation()# this finds new coordinates for the apple - Sanika R.
    apple4 = getRandomLocation()# this finds new coordinates for the apple - Sanika R.
    ...
    while True:  # main game loop
        ...
        if wormCoords[HEAD]['x'] == apple['x'] and wormCoords[HEAD]['y'] == apple['y']:
            ...
            apple = getRandomLocation()
            apple2 = getRandomLocation() #this creates a new location for the apple once the first snake hits the correct apple so that once apple2 is called, it will generate at a new location - Sanika R
            apple3 = getRandomLocation()#this creates a new location for the apple once the first snake hits the correct apple so that once apple2 is called, it will generate at a new location - Sanika R
            apple4 = getRandomLocation()#this creates a new location for the apple once the first snake hits the correct apple so that once apple2 is called, it will generate at a new location - Sanika R
            ...
            if worm2Coords[HEAD2]['x'] == apple['x'] and worm2Coords[HEAD2]['y'] == apple['y']:
                # don't remove worm's tail segment
                apple = getRandomLocation()
                apple2 = getRandomLocation() #this creates a new location for the apple once the second snake hits the correct apple so that once apple2 is called, it will generate at a new location - Sanika R
                apple3 = getRandomLocation() #this creates a new location for the apple once the second snake hits the correct apple so that once apple3 is called, it will generate at a new location- Sanika R
                apple4 = getRandomLocation() #this creates a new location for the apple once the second snake hits the correct apple so that once apple4 is called, it will generate at a new location- Sanika R
            else:
        ...
        drawApple(apple)
        drawApple(apple2) # This calls the drawApple function to draw the apple. It draws apple2 when it is called - Sanika R
        drawApple(apple3) # This calls the drawApple function to draw the apple. It draws apple2 when it is called - Sanika R
        drawApple(apple4) # This calls the drawApple function to draw the apple. It draws apple2 when it is called - Sanika R
...
def drawScore2(score):
    scoreSurf = BASICFONT.render('Score: %s' % (score), True, BLACK)
    scoreRect = scoreSurf.get_rect()
    scoreRect.topright = (WINDOWWIDTH - 120,
                          10)  # It also makes sure the score renders in a different location from the first score, preventing overlap -Anu
    DISPLAYSURF.blit(scoreSurf, scoreRect)
...
def drawWorm2(worm2Coords):
    for coord in worm2Coords:
        ...
        pygame.draw.rect(DISPLAYSURF, DARKGREEN, worm2SegmentRect)  # this draws dark green boxes for the worm's body at the location of the worm's coordinates (x and y) -Anu
